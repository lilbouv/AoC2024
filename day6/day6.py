...
for spot in previousGuardSpots:
    ...
    if len(previousGuardSpots) == 0:
        c_spot.append(spot)
        c+=1
#remove any from the starting guard position and any that have a negative index
startingPos = getGuardPosition()
if startingPos in c_spot[0]:
    c_spot.remove(startingPos)
if c_spot[0][0]:
    c_spot.remove((startingPos[0] - 1, startingPos[1]))

print(len(set([x[0] for x in c_spot])))